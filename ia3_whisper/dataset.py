...
class LibriSpeech(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item: int) -> dict[str, torch.Tensor | str]:
        ...
        padding_mask = torch.ones(mel.shape[-1], dtype=torch.bool)  # Shape: (3000,)
        # Mask out all positions with 0 std (== padding/no audio).
        padding_mask[mel.std(dim=0) == 0] = False
...
def get_dataloader(
    ...
    :param truncate: [Optional] Whether to truncate sequences to only non-padding tokens.
